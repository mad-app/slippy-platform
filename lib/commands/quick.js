...
function quick(url, options) {
  ...
  if (options.count && options.rate) {
    throw new Error('Error: either a fixed number of arrivals or arrivals per ' +
  ...
  if (options.rate) {
    ...
    throw new Error('Error: either arrival rate or an arrival count must be ' +
                'specified');
  }
  ...
  run(tmpf.name, {quiet: options.quiet, output: options.output, callback:options.callback});
}
